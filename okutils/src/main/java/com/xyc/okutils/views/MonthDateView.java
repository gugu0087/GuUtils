...
public class MonthDateView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        String dayString;
        int mMonthDays = DateUtils.getInstance().getMonthDays(mSelYear, mSelMonth);
        int weekNumber = DateUtils.getInstance().getFirstDayWeek(mSelYear, mSelMonth);
    ...
    public void onLeftClick(){
        ...
        if(month == 0){//若果是1月份，则变成12月份
            year = mSelYear-1;
            month = 11;
        }else if(DateUtils.getInstance().getMonthDays(year, month) == day){
            //如果当前日期为该月最后一点，当向前推的时候，就需要改变选中的日期
            month = month-1;
            day = DateUtils.getInstance().getMonthDays(year, month);
        }else{
            month = month-1;
        }
